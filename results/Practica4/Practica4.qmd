---
title: "PRACTICA4: Trabajar con tablas o marcos de datos con R"
lang: es
date: today
execute: 
  eval: true
...
# Preparación
En esta práctica trabajaremos en el entorno de R, con los paquetes siguientes: dplyr, kableExtra y jsonlite. Para descargar algunos datos de ejemplo, usaremos los programas datasets y dataformat (O’Leary et al. 2024), que pueden descargarse en este enlace.

# Objetivos
* Conocer el uso de datasets y dataformat.
* Ver algún ejemplo de archivo en formato JSON.
* Manipular marcos de datos sin usar hojas de cálculo.

# Datos
Supón que tenemos interés en descargar un genoma de referencia de cualquier especie del género Trypanosoma, así como su anotación funcional, en formato GFF3. Veamos primero de qué opciones disponemos:

```{bash}
DATA=/mnt/c/Documents/Bioinformatica/data/Practica4
mkdir -p "$DATA"

if [ ! -e "$DATA/trypanosoma_annotated.txt" ]; then
   datasets summary genome taxon Trypanosoma --annotated --as-json-lines | \
   dataformat tsv genome --fields \
      accession,organism-name,assminfo-release-date,assminfo-level,assmstats-atgc-count,annotinfo-featcount-gene-total,assminfo-type,assminfo-status > "$DATA/trypanosoma_annotated.txt"
fi
```
...
```{R}
library('kableExtra')
DATA_R <- "~/Bioinformatica/data/Practica4"

genomas <- read.table(file.path(DATA_R, 'trypanosoma_annotated.txt'),
                      header = TRUE, sep = '\t')
...
Si observas las líneas 7 y 8, verás que son registros idénticos (mismo Assembly Accession, misma fecha, etc.). Se trata de un registro duplicado en la base de datos descargada.

Busca una función de R que te permita identificar las líneas repetidas en un marco de datos.

La función de R diseñada específicamente para esto es duplicated(). Devuelve un vector lógico indicando qué filas son copias exactas de filas anteriores.

```{R}
genomas[duplicated(genomas), ]
```
...
DATA=/mnt/c/Documents/Bioinformatica/data/Practica4
mkdir -p "$DATA"
cd "$DATA" # Entramos en la carpeta para que el ZIP se descargue y descomprima allí

if [ ! -e ncbi_dataset/data/GCA_022059095.1/genomic.gff ]; then
   if [ ! -e ncbi_dataset.zip ]; then
      datasets download genome accession GCA_022059095.1 --include genome,gff3
   fi
   # Añadimos -o para sobrescribir sin preguntar en caso de existir parcialmente
...
library('jsonlite')
DATA_R <- "~/Bioinformatica/data/Practica4"
prettify(readLines(file.path(DATA_R, 'ncbi_dataset/data/assembly_data_report.jsonl')))
```
...
gff <- read.table(file.path(DATA_R, 'ncbi_dataset/data/GCA_022059095.1/genomic.gff'), 
                  sep = '\t',
...
Solución al Desafío
Una columna es invariable si al mirar sus valores únicos, solo hay 1. Por ejemplo, en los archivos GFF es muy habitual que el score o el source sean siempre iguales (como el valor .).
Podemos eliminar automáticamente de gene aquellas columnas que tienen un solo valor único:


```{r}
# Se seleccionan solo aquellas columnas que tengan más de 1 valor único
gene <- gene[, sapply(gene, function(x) length(unique(x)) > 1)]
head(gene, 2)
```
:::
...
Solución al Desafío
Para completarlo, primero debemos calcular la columna $longitud en los demás dataframes y, a continuación, quitar la coma sobrante y concatenar las variables en vectores:
```{r}
...
resumen_completo <- data.frame(
   ...
   número     = c(nrow(cds), nrow(gene), nrow(mrna), nrow(exon), nrow(region)),
   long.Media = c(mean(cds$longitud), mean(gene$longitud), mean(mrna$longitud), mean(exon$longitud), mean(region$longitud)),
   long.SD    = c(sd(cds$longitud), sd(gene$longitud), sd(mrna$longitud), sd(exon$longitud), sd(region$longitud))
)
kable(resumen_completo) %>% kable_styling(full_width = FALSE)
```
...
Solución al Desafío
Aprovechando dplyr, esto se resuelve agrupando primero por el ID del cromosoma (seqid) y el sentido (strand), y luego contando las filas:


```{r}
tabla_cromosomas <- gff %>% 
  filter(type == "gene") %>% 
  group_by(seqid, strand) %>% 
  summarise(Total_Genes = n(), .groups = 'drop')

head(tabla_cromosomas)
```
...
Solución al Desafío
Añadir la condición adicional utilizando el operador & (AND lógico):
```{r}
interesantes_genes <- gff[gff$seqid %in% candidatos & gff$type == "gene", ]
dim(interesantes_genes)
...
Solución al Desafío
Para que un gen solape o esté parcialmente contenido en un intervalo, su inicio debe ser menor o igual al final del intervalo (para no empezar después) y su final debe ser mayor o igual al inicio del intervalo (para no terminar antes).
Modificando el primer filtro como ejemplo:

```{r}
# Gen partially overlaps completely with interval 184638 to 253478
f1_parcial <- gff$seqid == 'JAJSEB010000048.1' & gff$start <= 253478 & gff$end >= 184638
```
...
Solución: Estrictamente hablando, en un archivo GFF (de 9 columnas) ninguna de las primeras 8 columnas es única de forma aislada. La verdadera llave única se encuentra codificada dentro de la 9ª columna (attributes), concretamente en el campo ID=, que proporciona un identificador unívoco y no repetible para cada elemento biológico anotado.
:::
...
Desafíos
¿Cómo comprobarías que al eliminar el sufijo de la versión, genomas$acc sigue siendo una llave de la tabla genomas?

Siguiendo el mismo esquema añade a genomas el “base_count” y compáralo con el “Assembly.Stats.ATGC.Count”. Si observas alguna diferencia, intenta explicarla.

Para comprobar que sigue siendo una llave perfecta y única, revisamos si existe algún duplicado en esa nueva columna:

```{r}
# Si devuelve FALSE, significa que todos los accesos recortados siguen siendo únicos
any(duplicated(genomas$acc))
```

Para añadir el conteo de bases y compararlo:


```{r}
genomas$base_count <- genomas.ena[match(genomas$acc, genomas.ena$accession), 'base_count']
head(genomas[, c('Assembly.Stats.ATGC.Count', 'base_count')])
```

Explicación: Si detectas diferencias en los números, suele deberse a cómo cada base de datos contabiliza la secuencia genómica. Por ejemplo, el ATGC.Count del NCBI reporta estrictamente la cantidad de nucleótidos A, T, G y C secuenciados (descontando los "huecos" o "N"). Por el contrario, el base_count del ENA a menudo refleja la longitud teórica total del ensamblaje, incluyendo las regiones con nucleótidos desconocidos (N) insertados artificialmente para rellenar vacíos.
:::
