...
```
[https://www.ebi.ac.uk/ena/portal/api/search?result=read_run&query=country=](https://www.ebi.ac.uk/ena/portal/api/search?result=read_run&query=country=)"United Kingdom" AND host_tax_id=9913 AND host_body_site="rumen"
```
...
```
[https://www.ebi.ac.uk/ena/portal/api/search?result=sequence&query=](https://www.ebi.ac.uk/ena/portal/api/search?result=sequence&query=)(specimen_voucher="ZMB:Moll:*" OR bio_material="ZMB:Moll:*")&fields=specimen_voucher,bio_material,scientific_name
```

```
[https://www.ebi.ac.uk/ena/portal/api/search?result=analysis&query=country=](https://www.ebi.ac.uk/ena/portal/api/search?result=analysis&query=country=)"United Kingdom" AND host_tax_id=9913 AND host_body_site="rumen" AND analysis_type="SEQUENCE_ASSEMBLY" AND assembly_type="primary metagenome"&fields=submitted_ftp
```

```
[https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157)](https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157)) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0
```
...
```{r}
#| error: true
# Primer bloque: Da error porque la URL contiene espacios (" AND "), y R necesita que estén codificados en formato URL (como %20).
curl_download('[https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157)](https://www.ebi.ac.uk/ena/portal/api/search?result=assembly&query=tax_tree(6157)) AND genome_representation="full"&fields=version,tax_id,scientific_name,last_updated,base_count&limit=0',
              destfile = 'z1.txt')
```

```{r}
# Segundo bloque: Funciona correctamente porque URLencode() arregla los espacios y caracteres especiales de la cadena completa. Pero es una línea demasiado larga e incómoda de leer/editar.
...
```{r}
# Tercer bloque: Es la mejor forma. Funciona bien y además el código está particionado en variables (portal, result, query...), lo que lo hace mucho más fácil de leer, mantener y reutilizar.
...
Este bloque no me funcionaba me ayudo gemini.
```{r}
portal <- 'https://www.ebi.ac.uk/ena/portal/api/search?'

result <- 'result=sequence&'
query  <- 'query=tax_tree(7215) AND description="alcohol dehydrogenase"&'
fields <- 'fields=tax_id,last_updated,description,base_count,scientific_name,first_public&'
limit  <- 'limit=0'

URL <- paste0(portal, result, query, fields, limit)

curl_download(URLencode(URL), destfile = 'z4.txt')


adh <- read.table('z4.txt', header = TRUE, sep = '\t', quote = '')

# 1. Cantidad de secuencias (número de filas en el dataframe)
n_secuencias <- nrow(adh)
cat("1. Hay", n_secuencias, "secuencias.\n")

# 2. Resumen de longitudes (columna base_count)
cat("2. Resumen de longitudes de las secuencias:\n")
summary(adh$base_count)

# 3. Número de especies diferentes (valores únicos en scientific_name)
n_especies <- length(unique(adh$scientific_name))
cat("3. Hay secuencias de", n_especies, "especies diferentes.\n")

# 4. Fecha de publicación más reciente
fechas_ordenadas <- sort(adh$first_public)
fecha_reciente <- tail(fechas_ordenadas, 1)
cat("4. La más reciente se publicó el", fecha_reciente, "\n")
```
...
```{r}
# Volvemos a construir la URL del ejercicio 1 para los platelmintos (URL limpia)
portal <- 'https://www.ebi.ac.uk/ena/portal/api/search?'
result <- 'result=assembly&'
query  <- 'query=tax_tree(6157) AND genome_representation="full"&'
fields <- 'fields=version,tax_id,scientific_name,last_updated,base_count&'
limit  <- 'limit=0'
URL_plat <- paste0(portal, result, query, fields, limit)

# Leemos la tabla SIN el colClasses para evitar el error de desajuste de columnas
platelmintos <- read.table(URLencode(URL_plat),
    header = TRUE,
    sep = '\t',
    na.strings = '')
```
...
**Respuesta:** Primero habría que ordenar el dataframe `platelmintos` cronológicamente usando la fecha de publicación (`last_updated`). Una vez ordenado, habría que crear una nueva variable/columna aplicando la función `cumsum()` sobre la columna `base_count` para obtener la suma acumulada de nucleótidos. Finalmente se usaría `plot()` con el tiempo en el eje X y la suma acumulada en el eje Y.
:::
...
formato   <- 'embl/'
accession <- 'AAC39011,AAY25307,ABU44510,ABG56046,AAO06902'
URL_embl <- paste0(portal, formato, accession)
